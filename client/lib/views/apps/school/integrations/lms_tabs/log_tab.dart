...
class _LogTabState extends State<LogTab> with UIMixin {
  ...
  void _onSearchChanged(String query) {
    if(controller.middlewareLogs.value?.data.isNotEmpty != false) {
      ...
      _debounce = Timer(const Duration(milliseconds: 500), () {
        controller.fetchMiddlewareLogs(controller.integrationData.value?.id ?? "", 1, query: query.trim());
      });
    }
  }
        ...
        children: [
          ...
          Obx(() {
            ...
            if(controller.isFetchingLogs.value) {
              return Center(
                child: LoadingAnimationWidget.staggeredDotsWave(color: contentTheme.onPrimary, size: 50),
              );
            }
                  ...
                  PaginationControls(
                    ...
                    onPageChanged: (newPage) {
                      controller.fetchMiddlewareLogs(
                        controller.integrationData.value!.id,
                        newPage,
                        isPageChange: true,
