...
async def get_platform_admin_user_counts():
    ...
    try:
        ...
        """
        role_counts = crud.fetch_all(role_counts_query)
        
        # Get total users
        total_users = crud.fetch_one("SELECT COUNT(*) as total_users FROM BINARY_SUCCESS_PLATFORM_USERS")
        
        # Get total schools
        total_schools = crud.fetch_one("SELECT COUNT(*) as total_schools FROM BINARY_SUCCESS_PLATFORM_INSTITUTES")
        
        # Get total students
        total_students = crud.fetch_one("SELECT COUNT(*) as total_students FROM BINARY_SUCCESS_LEARNERS")
        
        # Get total teachers
        total_teachers = crud.fetch_one("SELECT COUNT(*) as total_teachers FROM BINARY_SUCCESS_TEACHERS")
        
        # Get all roles
        all_roles = crud.fetch_all("SELECT role_id, role_name, role_description FROM BINARY_SUCCESS_ROLES ORDER BY role_name")
...
async def get_platform_dashboard(time_range_days: int = 30):
    ...
    try:
        ...
        """
        schools_result = crud.fetch_one(schools_query)
        ...
        """
        users_result = crud.fetch_one(users_query)
        ...
        """
        active_students_result = crud.fetch_all(active_students_query)
...
async def get_ai_usage_analytics(
    ...
    try:
        ...
        kpis = crud.fetch_one(kpis_query, params)
        ...
        trend_data = crud.fetch_all(trend_query, params)
...
async def get_performance_metrics(
    ...
    try:
        ...
        metrics = crud.fetch_all(query, params)
